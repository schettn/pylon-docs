...
<Callout type="warning" title="Warning">
  When using other databases, ensure proper type definitions for Pylon to maintain type safety and schema generation.
  When using the `any` type, Pylon will expose all data of the document to the schema, which may lead to security vulnerabilities.
</Callout>
